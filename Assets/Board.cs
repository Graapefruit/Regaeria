    ...
    public void setTileAsHovered(Vector3 point) {
        ...
        if (tileIndices == null) {
            if (currentlyHovered != null) {
                currentlyHovered.removeHighlightedStatus();
            }
            ...
            if (board[tileIndices.x, tileIndices.z] != currentlyHovered) {
                ...
                currentlyHovered.setAsHighlighted();
            }
        }
    }
    ...
    private void initializeBoard(GameObject tilePrefab) {
        for (int x = 0; x < BOARD_SIZE; x++) {
            for (int z = 0; z < BOARD_SIZE; z++) {
                ...
                board[x, z] = GameObject.Instantiate(tilePrefab, new Vector3(xStart, 0.001f, zStart), Quaternion.Euler(90.0f, 0.0f, 0.0f)).GetComponent<Tile>();
